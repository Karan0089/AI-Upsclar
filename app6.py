...
# ============================================================
# 2. ARCHITECTURE (Matches 16-Block MinecraftSpatialPro)
# ============================================================
...
def load_trained_model():
    ...
    model = MinecraftSpatialPro(num_features=64, num_blocks=16, scale=4, res_scale=0.1).to(device)

    weights_candidates = [
        "minecraft_spatial_pro_best.pth",
        "minecraft_spatial_pro_latest.pth"
    ]
    ...
    return model, device, loaded_file
...
comparison_target = st.sidebar.radio(
    "Left Slider View:",
    ["Trained AI (MinecraftSpatialPro)", f"Baseline ({selected_baseline.split()[0]})"]
)

if loaded_weights:
    st.sidebar.success(f"Weights: `{loaded_weights}` ({device})")
else:
    st.sidebar.warning("No checkpoint found. Running baseline comparisons only.")

# ============================================================
# 5. INFERENCE & EVALUATION ENGINE
# ============================================================
@st.cache_data
def evaluate_frame(image_name, baseline_flag_val):
    ...
    if loaded_weights and model is not None:
...
st.markdown("Evaluating **MinecraftSpatialPro (16 Blocks + VGG Perceptual + Sobel)** vs. Classical Baselines.")
...
if selected_image:
    ...
    if comparison_target.startswith("Trained AI") and loaded_weights:
        ...
        left_label = f"MinecraftSpatialPro ({latency:.1f} ms)"
    else:
